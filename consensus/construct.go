...
func (consensus *Consensus) construct(
	p msg_pb.MessageType, payloadForSignOverride []byte, blockHash []byte, pubKey *bls.PublicKey, priKey *bls.SecretKey,
	...
	if len(payloadForSignOverride) == 0 {
		...
		consensusMsg = consensus.populateMessageFields(
			message.GetConsensus(), blockHash[:], pubKey,
		)
	}
	...
	case msg_pb.MessageType_COMMIT:
		if s := priKey.SignHash(payloadForSignOverride); s != nil {
			consensusMsg.Payload = s.Serialize()
		}
